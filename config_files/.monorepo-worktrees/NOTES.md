...
Then: `wdc down -v` (drops the slot's `_build`/`deps`/`node_modules` volumes — going back to a deleted worktree is rare enough that a cold rebuild is the right trade; external volumes are untouched), `cd` back to the primary monorepo since the cwd ceases to exist, `git worktree remove`, delete the state dir to free the letter, and deal with the herdr workspace **last**, because closing the workspace the command runs in kills its own pane. Run from inside the doomed workspace, wdel first moves herdr to the first remaining worktree's workspace (state dirs glob in slot-letter order) — or, when no worktree workspaces remain, creates a fresh one at the primary monorepo with `--focus` — and only then closes the old workspace as its literal final statement, so the pane dying takes nothing with it. Run from anywhere else, it just closes the worktree's workspace without touching focus.
