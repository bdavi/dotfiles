...
################################################################################
# Unattended update_ubuntu.sh cron setup
################################################################################
...
# so update_ubuntu.sh can run unattended via cron, which has no TTY to
...
# Installs a system cron.d entry that runs update_ubuntu.sh unattended, so
...
install_update_cron_job() {
  ...
  local rule="$schedule $USER $DOTFILES_DIR/scripts/dev_box/update_ubuntu.sh >> $log_file 2>&1"
